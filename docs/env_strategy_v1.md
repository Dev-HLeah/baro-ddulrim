...
SUPABASE_CONTRACTOR_DOCUMENTS_BUCKET=""
...
- 신고 사진/영상 버킷명은 `SUPABASE_REPORT_ATTACHMENTS_BUCKET`에 둔다.
- 업체 사업자등록증 등 업체 문서 버킷명은 `SUPABASE_CONTRACTOR_DOCUMENTS_BUCKET`에 둔다.
- Prisma seed는 `ADMIN_SEED_EMAIL`, `ADMIN_SEED_PASSWORD`, `ADMIN_SEED_NAME`을 사용한다.
- 개발용 샘플 데이터는 `SEED_SAMPLE_DATA=true`일 때 생성한다.
