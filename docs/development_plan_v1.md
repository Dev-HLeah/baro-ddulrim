...
권장 Supabase Storage 버킷:

- 신고 사진/영상: `SUPABASE_REPORT_ATTACHMENTS_BUCKET`
- 업체 사업자등록증/업체 문서: `SUPABASE_CONTRACTOR_DOCUMENTS_BUCKET`

웹 고객 신고 화면은 사진 첨부를 지원한다. 실제 파일 업로드는 신고 생성 API에서 백엔드를 통해 Supabase Storage에 저장하고, 저장된 경로를 `report_attachments`에 기록한다.
